...
from links.villages import Village, VillageGovernance, VillagePolicy, save_village, add_member
from links.store import ingest_bundle_file, query_claims
from links.claims import read_bundle, verify_bundle

app = typer.Typer(help="Links: claim exchange + villages (access control + norms).")
villages_app = typer.Typer(help="Villages (Phase 4)")
claims_app = typer.Typer(help="Ingest/query bundles")
sync_app = typer.Typer(help="HTTP sync")
...
def villages_create(village_id: str, name: str, admin: str, description: str = "", visibility: str = "village", allowed_predicate: str = "links.weighted_to", max_window_days: int = 30):
    v = Village(
        ...
        policy=VillagePolicy(visibility=visibility, allowed_predicates=[allowed_predicate], max_window_days=max_window_days),
    )
    p = save_village(Path("data"), v)
    token = secrets.token_urlsafe(32)
    add_member(Path("data"), village_id, member_id=admin, role="admin", token_plain=token)
...
def villages_add_member(village_id: str, member_id: str, role: str = "member"):
    token = secrets.token_urlsafe(32)
    add_member(Path("data"), village_id, member_id=member_id, role=role, token_plain=token)
